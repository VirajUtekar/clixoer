...
public class HandbooksValidation extends base {
	...
	public void checkModuleExist()
	{
		...
		landingpage.getHandbooks().click();
		System.out.println("Interactives = "+ landingpage.getHandbooks().getText());
	}
...

	{
		...
		landingpage = new LandingPage(driver);
		landingpage.getHandbooks().click();
		String modulestext= landingpage.getHandbooks().getText();
		Assert.assertEquals(modulestext,"Handbooks 18");
		...
		Assert.assertEquals(titlenames.size(),18);
		}
